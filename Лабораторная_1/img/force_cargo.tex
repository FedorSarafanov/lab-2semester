...
\begin{tikzpicture}
    \draw[draw=none, use as bounding box] (-3.4mm,-35mm) rectangle ++(10mm,10mm);
	...
	\draw[->, blue] (rect.center) -- ++ (0,-3.5mm) node[right, scale=0.3] {$m\vec{g}$};
	\draw[->, blue] (rect.center) -- ++ (0,3.5mm) node[right, scale=0.3, xshift=0.3em] {$\vec{T}$};

	\draw[->, fill=black, black] (rect.center) circle (0.3pt);
