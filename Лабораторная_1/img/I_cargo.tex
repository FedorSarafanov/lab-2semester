...
\begin{tikzpicture}
    \draw[draw=none, use as bounding box] (-20mm,-20mm) rectangle (20mm,20mm);
	...
	\foreach \x in {0,90,...,270} {
	  ...
	  \draw[line width={31.5mm/10/2}, color=black!20] ($(0)+({\x+\iangle}:{6.3mm/10+42mm/10/2+8mm})$) -- +({\x+\iangle}:{25mm/10});
	...
	\draw[fill=black!30] (0) circle (10mm/10/2/2);		
	\end{scope}


	\draw[<->] (0,0) -- node[above, scale=0.7] {$r_1$} ++ (\iangle:12.2mm);
